...
use scroll_engine::{Engine, EngineDriver, EngineDriverEvent, ForkchoiceState};
use std::{
    io::Write,
    path::PathBuf,
    sync::Arc,
...
async fn skip_block_with_no_transactions() {
    ...
    tx.set_finalized_l1_block_number(0).await.unwrap();
    tx.commit().await.unwrap();

    // create a sequencer
    let config = SequencerConfig {
        chain_spec: node.inner.chain_spec().clone(),
...
async fn can_build_blocks() {
    ...
    // Set the finalized block number
    let tx = provider.tx_mut().await.unwrap();
    tx.set_finalized_l1_block_number(5).await.unwrap();
    tx.commit().await.unwrap();
...
async fn can_build_blocks_with_delayed_l1_messages() {
    ...
    // send a new payload attributes request.
    sequencer.start_payload_building(&mut engine);
    let block = if let SequencerEvent::PayloadReady(payload_id) = sequencer.next().await.unwrap() {
        let result = sequencer.finalize_payload_building(payload_id, &mut engine).await.unwrap();
        assert!(result.is_some(), "expected a new payload, but got: {:?}", result);
        result.unwrap()
...
//     sequencer.start_payload_building();
...
//     // sleep 2 seconds (ethereum header timestamp has granularity of seconds and proceeding
// header     // must have a greater timestamp than the last)
//     tokio::time::sleep(std::time::Duration::from_secs(2)).await;
...
//     if let Some(ChainOrchestratorEvent::SignerEvent(SignerEvent::SignedBlock {
//         block: signed_block,
//         signature,
...
//     // setup a test node. use a low payload building duration in order to exit before we reach
// the     // gas limit.
//     let (mut nodes, _tasks, wallet) = setup_engine(
//         ScrollRollupNodeConfig {
...
//     // sleep 1 seconds (ethereum header timestamp has granularity of seconds and proceeding
// header     // must have a greater timestamp than the last)
