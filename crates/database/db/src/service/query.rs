...
pub type BoxedFuture<T, Err> = Pin<Box<dyn Future<Output = Result<T, Err>> + Send>>;

/// A read query that uses a [`TX`] for the call.
pub type ReadQuery<T, Err> = Arc<dyn Fn(Arc<TX>) -> BoxedFuture<T, Err> + Send + Sync>;

/// A write query that uses a [`TXMut`] for the call.
pub type WriteQuery<T, Err> = Arc<dyn Fn(Arc<TXMut>) -> BoxedFuture<T, Err> + Send + Sync>;

/// A query to the database.
pub enum DatabaseQuery<T, Err> {
