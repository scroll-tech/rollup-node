...
use scroll_primitives::{BatchInput as BatchInputPrimitive, BatchInputV1, BatchInputV2};
...
impl From<BatchInputPrimitive> for ActiveModel {
    fn from(batch_input: BatchInputPrimitive) -> Self {
        match batch_input {
            BatchInputPrimitive::BatchInputV1(batch_input) => Self {
                index: ActiveValue::Set(
                    batch_input.index.try_into().expect("index should fit in i64"),
                ),
            ...
            BatchInputPrimitive::BatchInputV2(batch_input) => Self {
                index: ActiveValue::Set(
                    batch_input.batch_input_data.index.try_into().expect("index should fit in i64"),
                ),
                version: ActiveValue::Set(2),
                codec_version: ActiveValue::Set(batch_input.batch_input_data.version as u8),
                hash: ActiveValue::Set(batch_input.batch_input_data.hash.to_vec()),
                block_number: ActiveValue::Set(
                    batch_input
                        .batch_input_data
                        .block_number
...
impl From<Model> for BatchInputPrimitive {
    fn from(value: Model) -> Self {
        ...
        if value.version == 1 {
            BatchInputPrimitive::BatchInputV1(BatchInputV1 {
                index: value.index.try_into().expect("data persisted in database is valid"),
                version: value
                    .codec_version
                    .try_into()
            ...
            BatchInputPrimitive::BatchInputV2(BatchInputV2 {
