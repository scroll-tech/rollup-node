...
use rollup_node_primitives::{
    BatchCommitData, BatchInfo, L1MessageEnvelope, WithBatchInfo, WithFullL2Meta,
...
type DerivationPipelineFuture = Pin<
    Box<
        dyn Future<
                Output = Result<
                    Vec<WithFullL2Meta<ScrollPayloadAttributes>>,
...
/// Metadata necessary in the derivation pipeline in order to handle L1 reorgs and batch reverts.
type DerivationPipelineMetadata<T> = WithL1FinalizedBlockNumber<WithBatchInfo<T>>;
...
pub struct DerivationPipeline<P> {
    ...
    pipeline_future: Option<DerivationPipelineMetadata<DerivationPipelineFuture>>,
    ...
    batch_queue: VecDeque<WithL1FinalizedBlockNumber<Arc<BatchInfo>>>,
    /// The queue of polled attributes.
    attributes_queue: VecDeque<WithFullL2Meta<ScrollPayloadAttributes>>,
    ...
    P: L1Provider + Clone + Send + Sync + 'static,
{
    ...
    pub fn push_batch(&mut self, batch_info: BatchInfo, l1_block_number: u64) {
        ...
        self.batch_queue.push_back(WithL1FinalizedBlockNumber::new(l1_block_number, batch_info));
    ...
    fn handle_next_batch(
        &mut self,
    ) -> Option<DerivationPipelineMetadata<DerivationPipelineFuture>> {
        ...
        if let Some(info) = self.batch_queue.pop_front() {
            let block_number = info.l1_block;
            let index = info.inner.index;
            let hash = info.inner.hash;
            ...
            return Some(WithL1FinalizedBlockNumber::new(
                block_number,
                WithBatchInfo::new(index, hash, fut),
            ));
        }
        None
    }
    ...
    pub fn handle_reorg(&mut self, l1_block_number: u64) {
        self.batch_queue.retain(|batch| batch.l1_block <= l1_block_number);
        if let Some(fut) = &mut self.pipeline_future {
            if fut.l1_block > l1_block_number {
                self.pipeline_future = None;
            }
        }
        self.attributes_queue.retain(|attr| attr.l1_block <= l1_block_number);
    ...
    pub fn handle_batch_revert(&mut self, index: u64) {
        self.attributes_queue.retain(|attr| attr.index < index);
...
{
    type Item = WithFullL2Meta<ScrollPayloadAttributes>;
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let l1_finalized_block_number =
            this.pipeline_future.as_ref().map(|fut| fut.l1_block).unwrap_or_default();
        if let Some(Poll::Ready(res)) = this.pipeline_future.as_mut().map(|fut| fut.poll_unpin(cx))
        {
            match res {
                ...
                Err((batch_info, err)) => {
                    ...
                    this.batch_queue.push_front(WithL1FinalizedBlockNumber::new(
...
pub async fn derive<L1P: L1Provider + Sync + Send, L2P: BlockDataProvider + Sync + Send>(
    ...
    l1_v2_message_queue_start_index: u64,
    latest_processed_l2_block_number: u64,
) -> Result<Vec<WithFullL2Meta<ScrollPayloadAttributes>>, DerivationPipelineError> {
...
mod tests {
    ...
    async fn test_should_correctly_handle_batch_revert() -> eyre::Result<()> {
        ...
        let mut pipeline = DerivationPipeline {
            pipeline_future: Some(WithL1FinalizedBlockNumber::new(
                0,
                WithBatchInfo::new(10, B256::random(), Box::pin(async { Ok(vec![]) })),
            )),
            database: db,
            ...
            .into(),
            attributes_queue: [WithL1FinalizedBlockNumber::new(
                0,
                WithL2BlockNumber::new(
                    9,
                    WithBatchInfo::new(9, B256::random(), Default::default()),
                ),
            )]
            .into(),
            waker: None,
    ...
    async fn test_should_skip_l1_messages() -> eyre::Result<()> {
        ...
        let derived_l1_messages: Vec<_> = attributes
            .into_iter()
            .filter_map(|a| a.transactions.clone())
            .flatten()
            .filter_map(|rlp| {
    ...
    async fn new_test_pipeline() -> DerivationPipeline<MockL1Provider<Arc<Database>>> {
        ...
        let batches = (initial_block - 100..initial_block).map(|i| {
            WithL1FinalizedBlockNumber::new(i, Arc::new(BatchInfo::new(i, B256::random())))
        });
                ...
                WithL1FinalizedBlockNumber::new(
                    ...
                    WithL2BlockNumber::new(
                        ...
                        WithBatchInfo::new(
                            batch.index,
                            batch.hash,
                            ScrollPayloadAttributes::default(),
                        ),
                    ),
                )
            })
        ...
        DerivationPipeline {
            pipeline_future: Some(WithL1FinalizedBlockNumber::new(
                initial_block - 100,
                WithBatchInfo::new(initial_block, B256::random(), Box::pin(async { Ok(vec![]) })),
            )),
            database: db,
            l1_provider: mock_l1_provider,
            l1_v2_message_queue_start_index: u64::MAX,
