...
use scroll_codec::{Codec, decoding::payload::PayloadData};
...
pub struct DerivationPipeline<P> {
    ...
    /// The L2 block number associated with the last processed payload attribute.
    latest_processed_l2_block_number: u64,
}
...
impl<P: Debug> Debug for DerivationPipeline<P> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DerivationPipeline")
            ...
            .field("latest_processed_l2_block_number", &self.latest_processed_l2_block_number)
            .finish()
    }
}
    ...
    P: L1Provider + Clone + Send + Sync + 'static,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            latest_processed_l2_block_number: 0,
        }
    }
    ...
    pub fn push_batch(&mut self, batch_info: BatchInfo, l1_block_number: u64) {
        let batch_info = Arc::new(batch_info);
        self.batch_queue.push_back(WithFinalizedBlockNumber::new(l1_block_number, batch_info));
    ...
    fn handle_next_batch(&mut self) -> Option<WithFinalizedBatchInfo<DerivationPipelineFuture>> {
        ...
        let latest_processed_l2_block_number = self.latest_processed_l2_block_number;
        ...
        if let Some(info) = self.batch_queue.pop_front() {
            ...
            let fut = Box::pin(async move {
                ...
                let attrs = derive(
                    ...
                    tx,
                    l1_v2_message_queue_start_index,
                    latest_processed_l2_block_number,
                )
                .await
    ...
    P: L1Provider + Clone + Unpin + Send + Sync + 'static,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let l1_finalized_block_number =
            this.pipeline_future.as_ref().map(|f| f.number).unwrap_or_default();
        if let Some(Poll::Ready(res)) =
            this.pipeline_future.as_mut().map(|fut| fut.inner.poll_unpin(cx))
        {
            match res {
                Ok(attributes) => {
                    if let Some(attr) = attributes.last() &&
                        attr.l2_block_number > this.latest_processed_l2_block_number
                    {
                        this.latest_processed_l2_block_number = attr.l2_block_number
                    }

                    let attributes = attributes
                        .into_iter()
                        .map(|attr| WithFinalizedBlockNumber::new(l1_finalized_block_number, attr));
                ...
                Err((batch_info, err)) => {
                    ...
                    this.batch_queue.push_front(WithFinalizedBlockNumber::new(
...
pub async fn derive<L1P: L1Provider + Sync + Send, L2P: BlockDataProvider + Sync + Send>(
    ...
    l1_v2_message_queue_start_index: u64,
    latest_processed_l2_block_number: u64,
) -> Result<Vec<ScrollPayloadAttributesWithBatchInfo>, DerivationPipelineError> {
    ...
    let batch_info = BatchInfo::new(batch.index, batch.hash);
    ...
    let first_block_number = blocks.first().map(|b| b.context.number).unwrap_or_default();
    let mut expected_next_block = if latest_processed_l2_block_number == 0 {
        first_block_number
    } else {
        latest_processed_l2_block_number.saturating_add(1)
    };

    let mut attributes = Vec::with_capacity(blocks.len());
    for mut block in blocks {
        // check blocks are contiguous.
        if expected_next_block != block.context.number {
            return Err(DerivationPipelineError::BlockDiscontinuity {
                got: block.context.number,
        ...
        let attribute = ScrollPayloadAttributesWithBatchInfo::new(attribute, batch_info, number);
...
mod tests {
    ...
    use rollup_node_providers::{L1ProviderError, test_utils::MockL1Provider};
    use scroll_alloy_consensus::TxL1Message;
    use scroll_alloy_rpc_types_engine::BlockDataHint;
    use scroll_codec::decoding::test_utils::read_to_bytes;
    use scroll_db::{
        DatabaseTransactionProvider, DatabaseWriteOperations, test_utils::setup_test_db,
    };
    ...
    const L1_MESSAGE_INDEX_33: L1MessageEnvelope = L1MessageEnvelope {
        ...
        transaction: TxL1Message {
            ...
            input: bytes!(
                ""
            ),
        },
    };
    ...
    const L1_MESSAGE_INDEX_34: L1MessageEnvelope = L1MessageEnvelope {
        ...
        transaction: TxL1Message {
            ...
            sender: address!("7885BcBd5CeCEf1336b5300fb5186A12DDD8c478"),
            input: bytes!(
                ""
            ),
        },
    };
    ...
    async fn test_should_retry_on_derivation_error() -> eyre::Result<()> {
        ...
        let expected = ScrollPayloadAttributes {
            payload_attributes: PayloadAttributes { timestamp: 1696935657, ..Default::default() },
            transactions: Some(vec![bytes!(
                ""
            )]),
            no_tx_pool: true,
    ...
    async fn test_should_derive_calldata_batch() -> eyre::Result<()> {
        ...
        let attributes: Vec<_> = derive(batch_data, l1_provider, l2_provider, u64::MAX, 0).await?;
        let attribute = attributes
            .iter()
            .find(|a| a.payload_attributes.payload_attributes.timestamp == 1696935384)
            .unwrap();

        let expected = ScrollPayloadAttributes {
            payload_attributes: PayloadAttributes { timestamp: 1696935384, ..Default::default() },
            transactions: Some(vec![
                bytes!(
                    ""
                ),
                bytes!(
                    ""
                ),
            ]),
            no_tx_pool: true,
            block_data_hint: BlockDataHint::none(),
        ...
        assert_eq!(attribute.payload_attributes, expected);
    ...
    async fn test_should_skip_l1_messages() -> eyre::Result<()> {
        ...
        let l1_messages = vec![
            L1MessageEnvelope {
                ...
                transaction: TxL1Message {
                    ...
                    input: bytes!(
                        ""
                    ),
                },
            },
            L1MessageEnvelope {
            ...
            L1MessageEnvelope {
                ...
                transaction: TxL1Message {
                    ...
                    sender: address!("61d8d3E7F7c656493d1d76aAA1a836CEdfCBc27b"),
                    input: bytes!(
                        ""
                    ),
                },
            },
        ];
        ...
        let derived_l1_messages: Vec<_> = attributes
            .into_iter()
            .filter_map(|a| a.payload_attributes.transactions)
            .flatten()
            .filter_map(|rlp| {
